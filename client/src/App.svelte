<script>
  ...
  async function handleSetup(e) {
    ...
    try {
      ...
      storage.registerOrVerifyHostInCloud(setupName, setupPin).catch(() => {});
      await loadEvents();
    } catch (err) {
  ...
  async function handleUnlock(e) {
    ...
    if (isLocalEnvironment && superAdminPin && unlockPin.trim() === superAdminPin) {
    ...
    try {
      ...
      storage.registerOrVerifyHostInCloud(res.host_name, unlockPin).catch(() => {});
      unlockPin = "";
      await loadEvents();
    } catch (err) {
  ...
  async function loadEvents() {
    try {
      const currentHost = (authStatus?.host_name || "").trim().toLowerCase();
      const res = await api.getEvents(currentHost);
      events = res.events || [];

      // Clean up foreign events from other hosts that might have been mistakenly imported into local IndexedDB
      if (currentHost) {
        try {
          const allLocal = await db.events.toArray();
          for (const ev of allLocal) {
            if (ev.host_name && ev.host_name.trim().toLowerCase() !== currentHost) {
              await db.events.where('slug').equals(ev.slug).delete();
            } else if (!ev.host_name) {
              // Legacy local event: tag with current host so it remains owned by this host
              await db.events.where('slug').equals(ev.slug).modify({ host_name: authStatus.host_name || 'Host' });
            }
          }
        } catch (_) {}
      }

      // The cloud events table is authoritative for a signed-in host. This
      // removes an event from a host's local IndexedDB after Super Admin (or
      // another host device) has purged it from Supabase.
      let cloudEventSlugs = null;
      if (storage.isStorageConfigured() && currentHost) {
        try {
          const cloudEvents = await storage.getCloudEventsForHost(authStatus.host_name);
          if (Array.isArray(cloudEvents)) {
            cloudEventSlugs = new Set(cloudEvents.map((event) => event.slug));
            const localOwnedEvents = (await db.events.toArray()).filter(
              (event) => (event.host_name || "").trim().toLowerCase() === currentHost,
            );
            const staleSlugs = localOwnedEvents
              .filter((event) => !cloudEventSlugs.has(event.slug))
              .map((event) => event.slug);

            if (staleSlugs.length > 0) {
              await db.transaction("rw", [db.events, db.photos, db.guests, db.sync_logs], async () => {
                for (const staleSlug of staleSlugs) {
                  await db.events.where("slug").equals(staleSlug).delete();
                  await db.photos.where("event_slug").equals(staleSlug).delete();
                  await db.guests.where("event_slug").equals(staleSlug).delete();
                  await db.sync_logs.where("event_slug").equals(staleSlug).delete();
                }
              });
            }

            // A host logging in on a new device can hydrate its active cloud
            // events without relying on the Storage manifest as a directory.
            const localSlugs = new Set((await db.events.toArray()).map((event) => event.slug));
            for (const cloudEvent of cloudEvents) {
              if (!localSlugs.has(cloudEvent.slug)) {
                // Supabase uses a UUID primary key while Dexie owns its local
                // auto-increment key; retain the cloud fields but never copy
                // that UUID into IndexedDB's `id` column.
                const { id: _cloudEventId, ...cloudEventData } = cloudEvent;
                await db.events.put({
                  ...cloudEventData,
                  host_name: authStatus.host_name || "Host",
                  max_photos: Number(cloudEvent.max_photos) || 100,
                  created_at: cloudEvent.created_at || new Date().toISOString(),
                });
              }
            }

            const reconciled = await api.getEvents(currentHost);
            events = reconciled.events || [];
          }
        } catch (cloudReconcileErr) {
          // Do not remove local events when cloud access is unavailable.
          console.warn("Cloud event reconciliation error:", cloudReconcileErr);
        }
      }

      // Sync cloud manifests strictly matching THIS host's name (and capped at 10 events)
      if (storage.isStorageConfigured() && currentHost) {
        try {
          const globalList = await storage.listAllGlobalEventsFromStorage();
          if (globalList && globalList.length > 0) {
            const localSlugs = new Set(events.map(e => e.slug));
            let addedAny = false;
            for (const ge of globalList) {
              const geHost = (ge.host_name || "").trim().toLowerCase();
              // Only import if manifest's host_name matches THIS host, not another host!
              if (
                geHost &&
                geHost === currentHost &&
                (cloudEventSlugs === null || cloudEventSlugs.has(ge.slug)) &&
                !localSlugs.has(ge.slug) &&
                events.length < 10
              ) {
                await db.events.put({
                  slug: ge.slug,
                  name: ge.name,
                  host_name: authStatus.host_name || 'Host',
                  date: ge.date !== 'N/A' ? ge.date : new Date().toISOString().split('T')[0],
                  tagline: ge.tagline || 'Memories Shared in Real-Time',
                  moderation_enabled: true,
                  guest_upload_limit: 20,
                  max_photos: ge.max_photos || 100,
                  status: ge.status || 'active',
                  created_at: ge.created_at || new Date().toISOString()
                });
                addedAny = true;
              }
            }
            if (addedAny) {
              const updatedRes = await api.getEvents(currentHost);
              events = updatedRes.events || [];
            }
          }
        } catch (discErr) {
          console.warn("Storage events discovery error:", discErr);
        }
      }
        ...
        <div class="header-actions">
          {#if isLandingRoute}
            <nav class="landing-nav-links">
              ...
              <a href="#architecture" class="landing-nav-link">Architecture</a>
              <a
                href="https://ko-fi.com/deidi0"
                target="_blank"
                rel="noopener noreferrer"
                class="kofi-nav-btn"
                title="Support creator on Ko-fi"
              >
                <span>☕</span> <span class="kofi-nav-label">Ko-fi</span>
              </a>
              <button
          ...
          {#if isAppRoute && authStatus.is_authenticated}
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              class="btn-secondary btn-sm kofi-badge-btn"
              title="Support LuminaFeed on Ko-fi"
              style="display: inline-flex; align-items: center; gap: 0.35rem; text-decoration: none;"
            >
              <span>☕</span> <span class="kofi-badge-label">Ko-fi</span>
            </a>
            <button
  ...
  <main class="main-content {isSlideshowRoute ? 'slideshow-main' : ''}">
            ...
            <div class="guest-action-bar">
              ...
              {:else if guestEventData.status !== "archived"}
                <span class="quota-helper">
                  ...
                  {:else}
                    Remaining slots: <strong>{guestSession.quota.remaining}</strong> • Event total: <strong>{guestEventData.total_photos || 0} / 100</strong>
                  {/if}
                </span>
              {/if}
            </div>
      ...
      <div class="landing-page-wrapper">
        ...
        <section class="hero-section">
          <div class="hero-pill-badge">
            ...
            <span style="color: var(--color-primary); font-weight: 700;">v1.0 Ready</span>
          </div>
      ...
      {:else if !authStatus.initialized}
        <div class="card auth-card" style="margin: 3rem auto; text-align: left; max-width: 480px;">
          ...
          <p class="text-secondary" style="margin: 0.5rem 0 1.5rem 0; text-align: center;">
            Set up your host profile to create events, configure cloud storage, and moderate incoming photos.
          </p>
          ...
          <form onsubmit={handleSetup} class="form-stack">
            ...
            <button
              ...
              disabled={isSubmitting}
            >
              {isSubmitting ? "Setting up..." : "Start LuminaFeed"}
            </button>
          </form>
        </div>
    ...
    {:else if !authStatus.is_authenticated}
      <div class="card auth-card">
        ...
        <p class="text-secondary" style="margin: 0.5rem 0 1.5rem 0;">
          Enter your admin PIN to access the LuminaFeed management console.
        </p>
        ...
        <form onsubmit={handleUnlock} class="form-stack">
          ...
          <button
            type="button"
            class="btn-secondary btn-sm"
            style="width: 100%; margin-top: 0.5rem; opacity: 0.85; font-size: 0.8125rem;"
            onclick={() => {
              if (confirm("Forgot your PIN? This will reset the host session on this device so you can re-enter your Host Name and configure a new Admin PIN. Proceed?")) {
                handleResetHostSetup();
              }
            }}
          >
            Forgot PIN / Reset Host Setup
          </button>
        </form>
      </div>
