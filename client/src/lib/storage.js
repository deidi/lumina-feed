...
export async function loginHostInCloud(hostName, pinHash) {
  ...
  try {
    ...
    if (fetchErr || !existing) {
      return { success: false, error: `Host "${cleanHost}" not found. Please register first or check your spelling.` };
    }
    ...
    if (existing.pin_hash && existing.pin_hash !== pinHash) {
      return { success: false, error: 'Incorrect Admin PIN for this Host account.' };
    }
 ...
 */
export async function updateHostInCloud(oldHostName, updates = {}) {
 ...
 */
export async function deleteHostFromCloud(hostName) {
...
export async function registerOrVerifyGuestInCloud(eventSlug, guestName, pinHash = null) {
  if (!eventSlug || !guestName || !isStorageConfigured()) {
    const fallbackToken = 'guest_' + Math.random().toString(36).substring(2) + Date.now();
    return {
      success: true,
  ...
  try {
    ...
    const sessionToken = 'guest_' + Math.random().toString(36).substring(2) + Date.now();
