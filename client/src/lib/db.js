...
export async function ensurePhotoDecrypted(photo, keyString = '') {
  ...
  if (photo.thumb_blob) {
    ...
    return { ...photo, thumb_url: thumbUrl };
  }

  const thumbSourceUrl = photo.storage_thumb_url || photo.thumb_url;
  if (!thumbSourceUrl) return photo;

  const key = (keyString || getStoredEventKey(photo.event_slug) || '').trim();
  if (!key) return photo;
...
export async function getDecryptedOriginalBlob(photo, keyString = '') {
  ...
  try {
    ...
    const rawBlob = await res.blob();
    if (key) {
      return await decryptBlob(rawBlob, key, photo.mime_type || 'image/jpeg');
    }
    return rawBlob;
  } catch (err) {
    ...
    for (const cp of cloudPhotos) {
      ...
      if (match) {
        ...
        if (isEncrypted && eventKey && cp.storage_thumb_url) {
          ensurePhotoDecrypted({ id: addedId, ...newPhotoRecord }, eventKey).catch(() => {});
        }
      }
    }
