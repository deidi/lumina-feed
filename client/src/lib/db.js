...
export async function updateHostProfile({ host_name, current_pin, new_pin }) {
  ...
  if (isStorageConfigured()) {
    if (current_pin) {
      const currentHash = await sha256(current_pin.trim());
    ...
    await updateHostInCloud(currentHost, {
...
export async function uploadPhoto(slug, file, guestToken, options = {}) {
  ...
  if (isStorageConfigured()) {
    ...
    if (guest?.id) {
      await client.from('guests').update({ upload_count: (guest.upload_count || 0) + 1 }).eq('id', guest.id);
    }
  }
  ...
  try {
    ...
    if (photo) {
      ...
      if (photo.guest_token) {
        ...
        if (guest) {
          await client.from('guests').update({ upload_count: Math.max(0, (guest.upload_count || 1) - 1) }).eq('id', guest.id);
        }
      }
    }
