...
### Fixed
- **Mobile QR Guest Gallery Image Decryption**:
  - Resolved an issue where remote phone browsers joining via event QR code displayed image alt text instead of rendered photos.
  - Prioritized decrypted memory blobs (`thumb_blob`, `original_blob`) in `getPhotos()` over raw `.enc` Supabase Storage URLs.
  - Guarded `getPhotoSrc()` to prevent passing raw encrypted `.enc` binary data to browser `<img>` elements, automatically queuing client-side Web Crypto decryption.
  - Made `syncPhotosFromCloud()` await thumbnail batch decryption before UI hydration, preserving persistent Supabase CDN URLs in IndexedDB without transient blob URL overwrites.
  - Added reactive slide and gallery auto-decryption pipelines across guest live wall, host queues, and TV slideshow mode.
