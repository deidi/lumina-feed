...
### Added
- **Cryptographically Secure PRNG Token Generator**: Added `generateSecureToken(prefix)` utilizing Web Crypto API (`crypto.randomUUID()` / `crypto.getRandomValues()`) for all session tokens, guest connection identifiers, and storage filenames.
- **Supabase Storage Bucket Hardening**: Configured `luminafeed-photos` storage bucket with an explicit 5 MB max payload constraint and allowed MIME types (`image/jpeg`, `image/png`, `application/octet-stream`).
- **Database Row Level Security (RLS) Active TTL Guards**: Hardened `public.hosts` and `public.events` RLS policies to restrict operations to active 24-hour records (`expires_at > now()`).

### Changed
- **Direct Backend PIN Verification**: `loginHostInCloud` now filters directly by `pin_hash` without returning the hash over the network to the browser.
- **Host Mutation Verification**: Added PIN hash verification guards to `updateHostInCloud` and `deleteHostFromCloud`.
- **Header UI Layout**: Removed "Ko-fi" text label and relocated the creator support button as a clean ☕ icon button (`.kofi-header-btn`) positioned directly beside the dark/light mode toggle.

### Fixed
- **Private Key Exposure**: Removed bundled default RSA private key from the client bundle. Option B fallback decryption is now strictly handled at runtime via environment variables.
- **Dependency Vulnerabilities**: Fixed `devalue < 5.9.1` DoS vulnerability via `npm audit fix`.
...
### Fixed
- **Event Status Showing "🟢 Reopen Event" on Active Events**: The `public.events` table had no `status` column, so event records read back with `status: undefined` and the strict `=== "active"` check rendered the Reopen button on live events.
  - Added `status text not null default 'active'` to `supabase_schema.sql` and the 1-click SQL setup script (`get1ClickSQLSetupScript`), including a migration `ALTER TABLE` for existing databases. Hosts on older schemas should re-run the setup script (it is idempotent via `IF NOT EXISTS`).
  - Normalized `status: e.status || 'active'` on all read paths (`getEvents`, `validateAndFetchEvent`, realtime `gallery:synced` payload).
  - Hardened host toggle/label comparisons to `(status || "active")`.
  - `updateEventStatus` now uses schema-adaptive `safeSupabaseUpdate`, so Close/Reopen persists without hard-failing on older schemas.
...
### Added
- **Unified Synchronized Login Baseline**: Release baseline consolidating the v0.2.0 cloud-only migration.
