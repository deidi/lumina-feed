# 🚀 LuminaFeed v0.2.1 Official Release Notes
...
**LuminaFeed v0.2.1** delivers the unified synchronized login baseline for the zero-backend, cloud-first real-time ephemeral event photo sharing platform. Built as a high-performance Single Page Application (SPA) with **Svelte 5 Runes**, **Vite 6**, and **Vanilla CSS**, LuminaFeed requires no custom server maintenance, incurs zero hosting fees, and delivers an instant, private, and interactive photo sharing hub for weddings, conferences, parties, and corporate events with 100+ attendees.

This release represents a completely clean single-commit baseline incorporating optional client-side end-to-end encryption, automated 24-hour ephemeral lifecycles, direct-to-cloud media delivery, a 100% cloud-only Supabase architecture (no local database), unified host/guest authentication with passcode protection, cross-device attendee directories, live TV presentation slideshows, and multi-network real-time signaling.
...
### 👥 4. Unified Cloud Authentication & Guest Passcode Protection
- **Tabbed Host Portal**: Explicit **Sign In** (`loginHostInCloud`) and **Register** (`registerHostInCloud`) flows verified against `public.hosts` via `(host_name, pin_hash)`.
- **Guest Passcode (Option B)**: Optional 4-digit guest passcode (SHA-256 `pin_hash` in `public.guests`) with cross-device session restoration via `(event_slug, name, pin_hash)` (`registerOrVerifyGuestInCloud`).
- **Session Expiry Detection**: `getAuthStatus()` re-verifies sessions against the cloud database on every load and clears stale local tokens.
- **Schema-Adaptive Persistence**: `safeSupabaseUpsert` / `safeSupabaseUpdate` omit unsupported columns for backward compatibility with older Supabase schemas.
- **Cloud Event Validation**: `validateAndFetchEvent(slug)` checks Database + Storage manifests on every join and purges stale guest tokens for deleted events.

### 👥 5. Cross-Device Guest Directory & Attendance Tracking
...
### 💾 6. 100% Cloud-Only Architecture (No Local Database)
- **Supabase PostgreSQL + Storage Only**: Hosts, events, guests, and photo metadata live exclusively in the cloud; browser `localStorage` holds lightweight session tokens only.
- **Direct Cloud Uploads**: Photos stream straight to Supabase Storage (legacy IndexedDB `offline-queue.js` deprecated).
- **Cross-Device Consistency**: Identical data on GitHub Pages, localhost, and LAN origins with zero local-sync drift.

### 📱 7. In-Browser Image Optimization Pipeline
...
### 📺 8. Independent Multi-Tab TV Slideshow (`_blank`)
...
### 🛡️ 9. Real-Time Moderation Queue & 1-Click Auto-Approve
...
### 🗑️ 11. Resilient Multi-Layer Event Deletion
...
- **Cascading Foreign Key Safety**: Ensures orderly deletion (`photos` -> `guests` -> `events`) in the Supabase PostgreSQL database while safely pruning Supabase Storage folders, event manifests, and local session tokens.

### ☕ 12. Creator Support & Community Integration
...
| **Authentication** | Cloud-verified host login + guest passcode (`pin_hash`, schema-adaptive upserts) |
